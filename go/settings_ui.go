...
func (s *SettingsUI) ShowSettingsDialog() {
		...
		func(save bool) {
			if save {
				if err := s.handleSave(apiKeyEntry.Text, useLocalLLMCheck.Checked, 
					ollamaModelEntry.Text, ollamaEndpointEntry.Text); err != nil {
	...
	customDialog.Resize(fyne.NewSize(500, 450))
	customDialog.Show()
}
...
func (s *SettingsUI) handleSave(apiKey string, useLocal bool, model, endpoint string) error {
	// Update settings
