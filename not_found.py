...
def find_note_for_player(player_name, notes_dir):
    ...
    for note in os.listdir(notes_dir):
        note_name = (
            os.path.splitext(note)[0].replace("_", " ").title()
        )  # Remove the file extension and title-case it
...
def log_missing_notes(players_df, notes_dir):
    ...
    for _, row in players_df.iterrows():
        ...
        if (
            not note_name or score < 100
        ):  # Log only if no exact match or if fuzzy match isn't perfect
            closest_file = note_name if note_name else "N/A"
            not_found.append(
                {"Player": player, "Fuzzy Score": score, "Closest File": closest_file}
            )
