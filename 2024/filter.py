...
def get_latest_player_file(log_dir):
    files = [
        ...
        if f.startswith("players_") and f.endswith(".txt")
    ]
    if not files:
    ...
    latest_file = max(
        ...
        key=lambda f: datetime.strptime(
            f.split("_")[1].replace(".txt", ""), "%Y%m%d-%H%M%S"
        ),
    )
        ...
        with open(csv_file, "r") as file:
            ...
            for row in reader:
                ...
                if cleaned_name not in player_names:
                    ...
                    if (
                        adp < pick_number + 30
                        or ecr < pick_number + 30
                        or espn < pick_number + 30
                    ):
...
def main():
    ...
    if latest_file:
        ...
        if pick_number is not None:
            filtered_players = filter_players_from_csv(
                "combined_with_depth.csv", player_names, pick_number
            )
