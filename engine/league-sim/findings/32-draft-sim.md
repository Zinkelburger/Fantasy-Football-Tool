# 32 — Drafting by the model loses to ADP, and no narrow version helps

**Confidence: High.** 300 leagues × 6 seasons per arm, paired seeds,
13 arms. The sanity control lands at baseline; dose-response is
monotone.

## Question

Finding 25 (corrected) says the ADP board out-ranks the player model
on every position. Rank correlation is a list metric, not a league
outcome. So: in full simulated seasons, what does drafting from the
model cost or earn? And since the model is *independent* of ADP, does
blending it into the board buy anything?
...
- Identical in-season play for everyone: leak-free EWMA lineups, same
  waiver rules. The only difference between arms is the draft list.
...
## The number (pooled 2020–2025, 1800 leagues per arm)
...
- **Pure model is costly.** −.073 all-play vs BPA, title odds roughly
  halved (10.8% vs 20.7%), lost all 6 seasons. 2020 finished below
  the random-room baseline (.446).
- **Dose-response is monotone.** Every increment of model weight hurts
  (w=0 .595 → w=.25 .594 → w=.5 .580 → w=1 .522). The 25% tilt ties
  BPA on all-play/PF/playoffs (Δall-play −.001) and its −2.0pp title
  gap is ~2se: no benefit, possible small cost.
- Finding 25's +0.006±0.005 rank-correlation hint for board+model
  stacking does **not** cash into league outcomes.
- **BPA vs the family room is the real edge.** Pure ADP discipline (no
  reaching, sane roster fills, K last) is worth 2.5× baseline title
  odds against calibrated family behavior. The draft tool's job is
  delivering that discipline, not out-thinking the market.

## Round 2 — the narrow versions

The challenge: "ADP isn't even good, is there really no way to tilt
it?" Round 1 only rules out *uniform* interventions. These arms each
apply the model (or our findings) in one narrow place, everything else
BPA, same seeds:
...
**Every narrow arm is a tie with BPA; none beats it.** The arms that
clearly lose are the aggressive ones, same monotone pattern as round 1.

`blend_notop` is the only arm whose pooled means all sit on the good
side of BPA, so it got a paired test (`analysis/paired_arm_test.py`,
differencing sim-for-sim on the shared seed sequence — the arms face
the same room, seat and injury draws, which removes most of the
variance the pooled means carry):
...
A season of model tilt outside the top 12 is worth 1.6 points-for.
Nothing. (`HeroStats` now records per-sim `title_flags` so title rate
can be paired too.)

The `marks` arm is a friendly-test negative. The hot/cold threshold
and the WR/TE-only restriction were both chosen on 2017-25 data, which
includes these six target seasons. That arm drafted with mild
hindsight about which rule works and still did not beat taking the
board in order. Findings 16/17 remain true about *player scoring*.
What fails is the leap from "true" to "therefore move him on your
draft board," because ADP already moved him.

## Where our disagreements with ADP carry information
...
The late-board effect is positive in 5 of 6 seasons (2021 the
exception, −0.13; 2022 +0.40 and 2025 +0.34 carry it).

The information is real, and it lives **only past the point where the
market stops paying attention**. That is why round 1's uniform tilt
lost: it spent model error across the top of the board, where we are
worthless, to buy a small edge at the bottom.

`blend_late` (tilt only past rank 30) still did not convert that into
wins. Two reasons. By rank 30+ the per-player stakes are small — these
are bench and flex bodies whose season swings a point or two a game.
And in a 12x15 draft only a handful of picks land in that zone for any
one team. A real edge on cheap players is still a small edge.

## What a draft cannot test
...
2022/2025 tilts beat BPA; 2020/2023 they lose bigger. Year-level
noise, no exploitable pattern.
...
that name him) and at most a tiebreak between similarly priced
players — never a re-ranking, not even a partial one, not even at TE,
not even late, not even for rookies. The board page and cheat sheet
should sell the *discipline* edge, which is real and measured.

## Open door

The late-board disagreement signal is real (+0.164, 2.9se, 5 of 6
seasons) and has no room to pay off in a draft. Two places it could
still matter, both untested:
