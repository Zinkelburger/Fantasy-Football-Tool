...
**Confidence: High.** 8 seasons with cluster-bootstrap CIs; causal
swap test in sim v5, n=432 paired leagues per cell.

## The curves

We fit how many points the market's #k-ranked RB or WR actually
returns over a season. Both decline as you move down the board. RB
value falls **45% faster** than WR value.
...
Four things follow, each separately supported.
...
2. **The market already prices the level.** At the same *overall* ADP
   ...
   6 — nothing — hits zero around pick 31–46, and turns *negative* by
   pick 100 (−5.0 [−10.0, +0.6]). There is no cheap position to buy.
3. **The edge is the slope, not the level.** RB value decays 45%
   faster per ln-rank (−31 vs −21). Even at fair per-pick prices,
   order of operations matters: take the fast-depreciating asset
   first, because WR value is still on the shelf three rounds later
   and RB value is not. The sim confirms it causally — an
   otherwise-identical disciplined drafter forced to RB instead of WR
   at its round-1 pick gains **+0.017 ± 0.009 all-play**. Round 2:
   +0.015 ± 0.009. Round 3: +0.007 ± 0.009, noise, and by then the
   forced choice is ~RB10 vs ~WR6.
...
Same-ADP RBs miss ~0.65 more games than WRs. The "RBs get hurt" tax is
real, small, and flat in ADP. What decays is the per-game talent
premium: +1.0 ppg at pick 12, +0.2 by pick 48. Early RBs carry the
injury tax easily and late RBs don't. **Early RB is a good buy; late
RB is a bad buy.**

Busts differ in kind. RB busts are injury busts. WR busts are
performance busts — in rounds 4–6, 28% of WRs play 10+ games and still
finish below WR30, vs 19% of RBs. The wire restocks WR better too: 26%
of top-24 weekly WR scorers went undrafted vs 18% at RB. A dead WR
pick is easier to replace.
...
38% of their first-three picks are RB (finding 24).

In the family room the two are a statistical tie (robust_rb −
pick_value = +0.010 ± 0.012; RB×3-then-value = robust_rb −0.000 ±
0.008), because pick_value's math already spends 57% of its
first-three picks on RB.
...
within 0.4 pts, so the parametric curve is adequate. Same-ADP premium
from y ~ ln(ADP) × position with year fixed effects (within
transformation). Quantiles are kernel-weighted with bootstrap bands.
Causal check: `analysis/rb_wr_swap.py` — same drafter, same seeds,
pick forced RB vs WR in exactly one round. Bands/bust table:
`analysis/rb_wr_bands.py`. R² of rank on outcome is 0.18–0.20: rank
explains a fifth of the season. These are curves through a cloud,
which is why the CIs are the finding.
...
- ADP is FFC standard-format consensus, not the family's board. The
  family's RB-lateness makes the in-room premium slightly larger than
  the market numbers here.
- 2018 and 2025 have the steepest RB slopes (−40, −44). The premium
  breathes year to year; per-year slopes are printed by the script.
- Crossover CI is wide [20, 109]. Read it as "RB through the top-18,
  agnostic after rank ~24," not a sharp threshold at 38.
