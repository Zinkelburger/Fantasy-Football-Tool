...
**Confidence: High** (descriptive, full population of kickers 2020–2025)
...
rank, all six years pooled) is `figures/fig2_kicker_persistence.png` —
a shotgun blast with no visible diagonal.

Position-wide stakes are tiny anyway: the gap between K1 and K16 is
about **3 points per game** (see finding 07), the smallest spread of
any position.
...
- "ROS rank" = rank by PPG over the remaining weeks, minimum 6 games
  played (removes injury/cut noise — note this *helps* the hot-kicker
  case by excluding Shrader-type flameouts, and persistence is still
  absent).
...
- Median-rank persistence isn't literally zero — Tucker 2021 and
  Aubrey 2024 (elite kickers on strong offenses) held up. Knowing the
  *offense* has some value; knowing last week's box score has none.
- None of this says kickers score few points (K1 averaged 11.5 PPG,
  more than TE1). It says you cannot *select* the good one early, so
  paying anything for the attempt is waste.
