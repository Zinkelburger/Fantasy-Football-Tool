...
Team-strength ratings barely help fantasy.

Season-long, nothing beats last year's simple points-per-game, and
that only reaches a 0.42 correlation with next year. Team quality
churns too much to project a season on.

Week to week, knowing the opponent is worth about +1 point of accuracy
at QB and RB and nothing at WR. Wind hurts QBs (−1.8) and leaves RBs
alone. The WR-vs-cornerback matchup data everyone sells is noise — it
doesn't even agree with itself.

The Vegas line already prices nearly all of it.
...
least squares). Sanity check: our Elo correlates 0.847 with Vegas
closing spreads and predicts margins within 0.5 MAE of the closing
line (10.75 vs 10.23). Close behind the market, never ahead of it.
...
real but it churns. A player model already embeds most of it through
the player's own production.

## Season-long: environment features add nothing
...
Per-position deltas ±0.005, coefficient signs unstable. Kept in the
script as a documented null.

## Weekly: the "bad defense" model

Panel: 36,246 player-weeks (2018–2025, ≥2 prior appearances), target =
that week's fantasy points. The claim itself, descriptively — average
points vs stingiest → softest quartile of defense-vs-position:

| | QB | RB | WR | TE |
|---|---|---|---|---|
| matchup spread | **+1.3** | **+1.2** | +0.3 | +0.4 |

Real but small, and near-zero at WR. Then the nested models, LOYO rank
...
2. **Opponent and Vegas only matter at QB** (+.028 over usage). QB
   ...
   +0.7. Matchup thinking pays where finding 06's streamers live and
   nowhere else.
...
## Weather: wind is the one that bites
...
**Bad weather doesn't make RBs better. It makes everyone else worse.**
That is the true core of the "snow games make RBs unstoppable"
folklore. Wind is the variable that bites (passing efficiency); cold
alone is mild.

Adding weather features to the weekly model (M5) moves QB from .434 to
.436 and nothing else. **Vegas implied totals already price the
forecast** — by the time you see 20mph wind, the total has moved. Use
weather through the implied total, and for the RB-relative tilt in
truly bad games.

## WR/CB matchups: the signal isn't there
...
**We also scored their actual product.** A full-slate snapshot of the
report (2025 week 5, via wayback; hand-transcribed, 77 active WRs,
`analysis/fp_matchup_test.py`) against that week's real points. Their
**MATCHUP score correlates −0.03 with actual scoring**, and the "bad
matchup" quartile *outscored* the "good matchup" quartile 7.5 to 6.2.
Their ADVANTAGE score does correlate (0.45), but entirely through its
skill component: their own plain columns beat it (FP/RR 0.48,
**route% 0.50** — literally "how many routes does he run" was the best
predictor on the page). Adding MATCHUP to a skill-only baseline moves
R² 0.297 → 0.304 with a wrong-signed coefficient.

One week can only detect |r| > ~0.2. Three independent tests now
agree: team DvP ≈ +0.3 pts, directional reliability ≈ 0.1, their
product ≈ 0.0. Detecting r = 0.1 would need ~10 more hand-transcribed
weeks and still wouldn't change a start/sit decision. Volume and form
are the signal. The matchup layer is noise.

The McLaurin-vs-shadow-corner story is the best version of the idea: a
*charted* elite shadow CB erasing an outside-only WR. That is
...
- **nfelo is the best free team rating.** MAE 10.09 vs the closing
  line's 10.11 on 3,929 games; our simple Elo is 10.75. Use nfelo when
  a team rating is needed.
- **…and it still doesn't matter for fantasy.** Next-season PF/g corr:
  nfelo 0.376 = our Elo 0.377 < naive last-year PF 0.419. The ~0.4
  wall above is not a rating-quality problem — team scoring churns on
  things no rating carries.
- **QB-Elo is the one usable signal.** Same-season corr with QB
  fantasy PPG = 0.78 — a live QB quality index. Start-of-season value
  predicts that season's PPG at 0.554, vs 0.538 for last-year PPG;
  partial beyond last-year PPG +0.12 (n=187, ~1.7σ), so weak-positive
  as a draft signal. The per-game `qb_adj` columns quantify
  starter-vs-backup downgrades, which is streaming/opponent context
  our sim currently ignores (backlog B15).
- **Preseason win-total lines beat every stat-based environment
  signal** — corr 0.458 with that season's PF/g vs 0.421 for lag PF.
  The market's offseason-aware projection is the best team prior. The
  dataset ends 2022, and season-long environment features add
  ~nothing to the player model regardless, so this is a reference
  point, not a feature.
...
## What we have now, and what we still lack
