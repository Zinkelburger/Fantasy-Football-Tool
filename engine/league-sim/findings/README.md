...
| [33](33-te-same-pick.md) | The same-pick test: early TEs (R1-4, 2018-25) beat the WR/RBs drafted at the same picks 69% of the time and bust at half the rate (8% vs 20-24%); ex-Kelce the edge shrinks to +6 pts but never flips. TE hit odds cliff after R4 (86→53→30→16% top-6) — elite door or round 10+, never rounds 5-9; edge grows std→PPR | data | Medium-High |
| [34](34-roster-shape.md) | Roster shape is a null **only if you use the wire**: every template (incl. 1QB-9RB-3WR-1TE-1K, QB2, TE2, WR-heavy) is within 9 pts/season of free value-drafting at ±6, and a QB2 is worth +0.1 despite an empty QB slot costing 14.3 — the wire patches it free. Turn the wire off and every template drops to −2.4…−21.5: **RB-hoarding goes best→worst** (−0.2→−21.5) while QB2/TE2 barely move (−2.5/−3.2). Lopsided rosters are wire-dependent rosters; the wire-off arm independently reproduces finding 22's −31 | sim v5 | Medium-High |
