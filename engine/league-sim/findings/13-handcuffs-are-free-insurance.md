...
**Confidence: Medium-High.** Simulated in two environments, n=1,440
seasons each. The effect is a precise zero, not an imprecise maybe.

Should you draft your starting RB's backup — his "handcuff" — as
injury insurance? The insurance is real. It is also fairly priced, so
it buys no edge.

Handcuffs deliver starter-level production in exactly the weeks your
starter is out: 9.5 PPG, about half those weeks at RB1 level. But a
drafter that pays a premium to grab its own handcuffs ties the one
that doesn't — **.598 vs .599 all-play**. The payout is cancelled
point-for-point by the picks it gave up. Take the handcuff when it's
a coin-flip pick anyway. Never reach for one.

## What handcuffs produce (2020–2025)
...
of them saw the starter miss at least one week the cuff played.
...
The case that started the question — Bucky Irving 2025, missed 8
weeks: Rachaad White gave **7.7 PPG in the Bucky-out weeks** vs 3.6
otherwise. Functioned as designed, mediocre payout, and front-loaded:
...
Strategy `pick_value_hc` (`simfl/strategies.py`) is identical to
`pick_value`, except that from round 9 the handcuff of an
already-rostered top-72 RB is scored at full lineup weight plus a
10-season-point insurance bonus. 240 sims × 6 seasons per cell,
seat-rotated, common random numbers vs the plain drafter.
...
**Environment v3** (adds news-aware projections, so *everyone* starts
and claims promoted backups immediately):
...
−.002. A dead tie. The v2 penalty was partly projection lag; with
realistic news-reading the premium is priced exactly right.
...
The variance argument (insurance → titles) had the right sign in v2
(+0.7pp titles) and vanished in v3 once opponents could also claim
promoted backups off waivers. Both effects sit within the ±1.1pp
title CI.
...
projection, for lineup decisions **and** FA/waiver valuation. Sanity
case: Rachaad White's week-5–12 projection rises 4.0 → 10.3 while
Bucky is out. This upgrade compressed every hero's edge by ~0.8pp —
a sharper field — without changing any strategy ordering.
...
Findings 01–12 cite v1 numbers. Orderings replicate in v2/v3; levels
differ (robust_rb .518 → .607 → .598). Rerun any of them against
`results/` before putting a level — not an ordering — on a slide.

Three measured artifacts of v3/v4 drove v5's waiver fixes: the
